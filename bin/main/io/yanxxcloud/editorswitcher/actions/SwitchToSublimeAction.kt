...
class SwitchToSublimeAction : AnAction() {

    override fun actionPerformed(e: AnActionEvent) {
        val context = EditorUtils.getEditorContext(e)
        val service = EditorSwitcherService.getInstance()
        
        if (service.sublimePath.isEmpty()) {
            service.detectEditorPaths()
            if (service.sublimePath.isEmpty()) {
                Messages.showErrorDialog(
                    context.project,
                    "Sublime Text 路径未配置。请在 Settings > Tools > Smart Editor Switcher 中配置路径。",
        ...
        Messages.showInfoMessage(
            context.project,
            "正在切换到 Sublime Text... (行: ${context.line}, 列: ${context.column})",
            "Smart Editor Switcher"
        )
    }
